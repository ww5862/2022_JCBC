...
public class EmployeeTable extends JPanel {
	...
	private JCheckBox pname_b = new JCheckBox("Pname", true);
	...
	//hyein
	private final String sorting[] = { "정렬 없음", "NAME 순", "SSN 순", "BDATE 순", "SALARY 순", "PNAME 순" };
	...
	private class EmployeeTableHandler implements ActionListener {
		...
		private String selectWhere(ActionEvent e) {
			...
			if (setSelectW.getSelectedItem() == "전체")
				return "";
		...
		private void selectSearch(ActionEvent e) {
			if ((name_b.isSelected() || ssn_b.isSelected() || bdate_b.isSelected() || address_b.isSelected()
					...
					|| department_b.isSelected()) && pname_b.isSelected()) { // 선택을 할 시에만 검색을 한다.
				// sql conn 호출
				...
				if (name_b.isSelected()) {
					if (!ssn_b.isSelected() && !bdate_b.isSelected() && !address_b.isSelected() && !sex_b.isSelected()
							&& !salary_b.isSelected() && !supervisor_b.isSelected() && !department_b.isSelected() && !pname_b.isSelected())
				...
				if (bdate_b.isSelected()) {
					if (!address_b.isSelected() && !sex_b.isSelected() && !salary_b.isSelected()
							&& !supervisor_b.isSelected() && !department_b.isSelected() && !pname_b.isSelected())
						sql += "e.Bdate ";
					else
						sql += "e.Bdate, ";
					header.add("BDATE");
				...
				if (address_b.isSelected()) {
					if (!sex_b.isSelected() && !salary_b.isSelected() && !supervisor_b.isSelected()
							&& !department_b.isSelected() && !pname_b.isSelected())
						sql += "e.Address ";
					else
						sql += "e.Address, ";
					header.add("ADDRESS");
				...
				if (sex_b.isSelected()) {
					if (!salary_b.isSelected() && !supervisor_b.isSelected() && !department_b.isSelected() && !pname_b.isSelected())
						sql += "e.Sex ";
					else
						sql += "e.Sex, ";
					header.add("SEX");
				...
				if (salary_b.isSelected()) {
					if (!supervisor_b.isSelected() && !department_b.isSelected() && !pname_b.isSelected())
						sql += "e.Salary ";
					else
						sql += "e.Salary, ";
					header.add("Salary");
				...
				if (supervisor_b.isSelected()) {
					if (!department_b.isSelected() && !pname_b.isSelected())
				...
				sql += "from employee e left outer join employee s on e.super_ssn=s.ssn, Department d, works_on w left outer join project p on w.Pno = p.Pnumber where e.Dno = d.Dnumber and e.Ssn = w.Essn";
				sql += selectWhere(e);
				//hyein
				sql += order_by;
			...
			if (!pname_b.isSelected()) {
				...
				sql += "from employee e left outer join employee s on e.super_ssn=s.ssn, Department d where e.Dno = d.Dnumber";
				sql += selectWhere(e);
				//hyein
				sql += order_by;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == setSorting) {
				...
				if (setSorting.getSelectedItem() == "PNAME 순"){
					order_by = " ORDER BY Pname";
				}
			}
